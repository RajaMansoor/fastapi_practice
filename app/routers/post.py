...
limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    ...
    models.Post.title.contains).limit(limit).offset(skip).all()

    return posts

# for posting a dict of data/ creating post
...
Depends(oauth2.get_current_user)):
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail= f"this id: {id} does'nt exist")
    return post 
...
Depends(oauth2.get_current_user)):
    ...
    if post == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail = f"{id} does'nt exist")
    
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail= "unathourized to delete")
...
Depends(oauth2.get_current_user)):
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail= "unathourized to update")
